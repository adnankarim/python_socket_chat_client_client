...
is_sel=False #boolean is user selected for chat
SERVER_IP='127.0.0.1'
SERVER_PORT=5555
...
def authenticator():
    ...
    while not isConn:
        ...
        PASS=get_string('Enter password:')
...
def get_number(string):
    ...
    if not number.isdigit():
        get_string('Enter a number!' + string)
    number = str(number)        
    return int(float(number.strip()))
...
def client_receive():
    ...
    while True:
        try:
            ...
            elif (str(message[1])== CHATTING_WITH and str(message[2])==USER) or str(message[2])=='ALL':
...
def client_send():
    """
    Send function fro client,will work in a separate thread and checks are ended:
    ...
    global is_sel
    while True:
        if not CHATTING_WITH and not is_sel:
            is_sel=select_user_for_chat()
            #is_sel is True if user selected
        elif is_sel==True:
            input_msg=input("")
